...
class SpectrogramParser(AudioParser):
    ...
    def parse_audio(self, audio_path):
        ...
        if self.normalize:
            ...
            spect.add_(-mean)
            spect.div_(std)
        # print('lllllllllllllllllll',spect.shape)
...
class SpectrogramDataset(Dataset, SpectrogramParser):
    def __init__(self, audio_conf, manifest_filepath_list, label2id, normalize=False, augment=False):
        ...
        self.max_size = 0
        self.ids_list = []
        manifest_filepath_list = [manifest_filepath_list]
    ...
    def __getitem__(self, index):
        ...
        if len(sample) == 2:
            ...
            spect = self.parse_audio(audio_path)#参照masr模型的输入
    ...
    def parse_transcript(self, transcript_path):
        ...
        transcript1 = constant.SOS_CHAR + transcript_path.lower() + constant.EOS_CHAR
        transcript = list(filter(None, [self.label2id.get(x) for x in list(transcript1)]))
        return transcript
...
def _collate_fn(batch):
    ...
    targets2 = torch.IntTensor(targets2)
    device = torch.device("cuda:3" if torch.cuda.is_available() else "cpu")
    # print('data loader :',input_sizes)
    # return inputs, targets, input_percentages, input_sizes, target_sizes,targets2
    return inputs.to(device), targets.to(device), input_percentages.to(device), input_sizes.to(device), target_sizes.to(device),targets2.to(device)
