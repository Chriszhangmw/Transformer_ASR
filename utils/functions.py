...
def save_model(model, epoch, opt, metrics, label2id, id2label, best_model=False):
    ...
    """
    leyerInfo = str(constant.args.num_layers) + '_' + str(constant.args.num_heads)
    if best_model:
        save_path = "{}/{}/{}/best_model.th".format(
            constant.args.save_folder, constant.args.name,leyerInfo )
    else:
        save_path = "{}/{}/epoch_{}.th".format(constant.args.save_folder,
                                               constant.args.name, epoch)

    if not os.path.exists(constant.args.save_folder + "/" + constant.args.name + "/" + leyerInfo):
        os.makedirs(constant.args.save_folder + "/" + constant.args.name + "/"+leyerInfo)
